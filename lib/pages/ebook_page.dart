...
class EBookPage extends StatelessWidget {
                          ...
                          itemBuilder: (_, index1) {
                            return BookAndTitleWidget(
                              ...
                              onTap1: () {
                                _showBottomSheet(
                                    context,
                                    listVO?[index].books?[index1] ?? BooksVO(),
                                    libraryModel);
                              },
                              onTap: () {
                                context
                                    .read<HomePageBloc>()
                                    .saveBookDataInDetail(
                                        listVO?[index].books?[index1]);
                                Navigator.of(context).push(MaterialPageRoute(
                                    builder: (context) =>
                                        ChangeNotifierProvider<
                                            BookDetailPageBloc>(
                                          create: (_) => BookDetailPageBloc(
                                              "${listVO?[index].books?[index1].title}"),
  ...
  Future _showBottomSheet(
      BuildContext context, BooksVO booksVO, LibraryModel libraryModel) {
    return showModalBottomSheet(
        context: context,
        builder: (context) {
          return BottomSheetWidget(
            image: '${booksVO.bookImage}',
            title: '${booksVO.title}',
            onTap: () {
              Navigator.pop(context);
              ShelfPageBloc bloc = context.read<ShelfPageBloc>();
              bloc.shelfVO =
                  context.read<HomePageBloc>().saveBookDataInShelfVO(booksVO);
              Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => const YourShelfPage()));
            },
            onTapLib: () {
              Navigator.pop(context);
  ...
  void _showSnackBar(context, BooksVO? booksVO) {
    final snackBar =
        SnackBar(content: Text("Delete ${booksVO?.title} From Library"));
